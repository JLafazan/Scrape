...
function createArticle(data) {
  var articleHtml = "<p data-id='" + data._id + "'>" + data.title + "<br />" + data.link + "</p>"
  console.log(data)
  var note = data.note
  var noteHtml = note ? `<span>NOTE:${note.title}: ${note.body}</span>` : ""

  


  var html = articleHtml + noteHtml
...
$.getJSON("/articles", function(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    $("#articles").append(createArticle(data[i]));
  }
});
